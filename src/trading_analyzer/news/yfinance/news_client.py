...
class YFNewsClient:
    ...
    def get_news(
        ...
        try:
            ...
            for article in news:
                ...
                content = article.get(
                    "content", article
                )  # Fallback to article if no content key
                if content is None:
                    continue
                    ...
                    try:
                        ...
                        if isinstance(formatted_article["published_date"], str):
                            ...
                            dt = datetime.fromisoformat(
                                formatted_article["published_date"].replace(
                                    "Z", "+00:00"
                                )
                            )
                            formatted_article["published_date"] = dt.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )
                    except Exception:
    ...
    def search_news_by_keyword(
        self, symbol: str, keyword: str, limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(
                f"Error searching news for {symbol} with keyword '{keyword}': {str(e)}"
            )
